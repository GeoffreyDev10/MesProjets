...
func lireAsciiDepuisFichier(fichier string) (map[rune]string, error) {
	file, err := os.Open(fichier)
	if err != nil {
		return nil, err
	}
	...
	for scanner.Scan() {
		...
		if len(ligne) > 1 {
			char := rune(ligne[0])
			ascii := ligne[2:]
			asciiMap[char] = ascii // sert à enregistrer dans la map la lettre
		}
	}
...
func affichAscii(lettresTrouvees []rune, asciiMap map[rune]string) string {
	result := ""
	for _, l := range lettresTrouvees {
		if l == '_' {
			result += "_" // caractère caché, on garde le "_"
		} else if ascii, exists := asciiMap[l]; exists {
			result += ascii + " " // affiche la version spéciale
		} else {
			result += string(l) + " " // sinon affiche la normale
		}
	}
	return result
}
...
func main() {
	mots, err := lireMotsDepuisFichier("words2.txt")
	if err != nil {
